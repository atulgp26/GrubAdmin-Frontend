...
export default function Modal({
  ...
  // Add !important utility classes if needed
  ...
  return createPortal(
    <div className={`fixed inset-0 ${top} ${right} ${bottom} ${left} z-50 flex ${positionClassWithImportant} shadow-lg ${noBlur ? "" : "backdrop-blur-sm"} ${customClass}`}>
      <div
        ref={modalRef}
      ...
      >
        ...
        <div className={`flex flex-col flex-1 justify-center h-full ${noXPadding?"px-0 py-0":"px-6 py-0 mt-10 mb-6"}`}>
          {children}
        </div>
      </div>
    </div>,
