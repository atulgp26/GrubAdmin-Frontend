...
export default function EditProfileModal({
  ...
  useEffect(() => {
    if (open && fields) {
      ...
      const nameParts = (fields.name || '').trim().split(' ').filter(part => part.length > 0);
  ...
  const roleOptions = [
    ...
    { id: "relationmanager", label: "Relation manager", description: "10 permissions" },
    { id: "support", label: "Support", description: "10 permissions" },
    { id: "technician", label: "Technician", description: "10 permissions" },
  ...
  const prepareProfileData = (fields) => {
    ...
    if (fields.name && fields.name.trim()) {
      ...
      // Fallback to formData if name is not in fields
      if (formData.firstName) {
        profileData.first_name = formData.firstName.trim();
      }
      if (formData.lastName) {
        profileData.last_name = formData.lastName.trim();
      } else {
    ...
    if (fields.facility && fields.facility.trim()) {
      profileData.location = fields.facility.trim();
    ...
    if (formData && formData.joiningDate) {
      try {
        ...
        if (!isNaN(dateObj.getTime())) {
          profileData.joining_date = dateObj.toISOString();
        }
      } catch (error) {
        console.error("Error parsing joining date:", error);
      }
  ...
  const handleSaveChanges = () => {
    ...
    if (editingField && tempValue !== currentFields[editingField]) {
      ...
      if (editingField === "email" || newEditedFields.has("email")) {
        setOtpEmail(editingField === "email" ? tempValue : newFields.email);
        setTitle("Hold on!");
        setDescription(`To confirm changing your email address, enter the OTP sent to your updated ID ${newFields.email}`);
        ...
        startOtpTimer();
        return;
      } else if (editingField === "contact" || newEditedFields.has("contact")) {
        setOtpEmail(editingField === "contact" ? tempValue : newFields.contact);
        setTitle("Hold on!");
        setDescription(`To confirm changing your contact number, enter the OTP sent to your updated number ${newFields.contact}`);
        // Prepare pending profile data
        const updatedFields = { ...newFields };
        if (formData.firstName && formData.lastName) {
          updatedFields.name = `${formData.firstName} ${formData.lastName}`;
    ...
    if (formData.firstName && formData.lastName) {
      finalFields.name = `${formData.firstName} ${formData.lastName}`;
    }
    if (formData.location) {
      finalFields.facility = formData.location;
    }
    ...
    if (editedFields.size > 0 || formData.firstName || formData.lastName || formData.location || formData.joiningDate) {
      setTitle("Hold on!");
      setDescription("To confirm your profile changes, enter the OTP sent to your registered email");
  ...
  const handleOtpVerify = async () => {
    ...
    try {
      ...
      if (pendingProfileData && Object.keys(pendingProfileData).length > 0) {
        // Log the data being sent for debugging
        console.log("Sending profile data:", pendingProfileData);

        const patchResponse = await accountService.patchProfile(pendingProfileData);
        ...
        if (patchResponse.success && patchResponse.code === 200) {
          ...
          const errorMsg = patchResponse.message || patchResponse.error || "Failed to update profile. Please try again.";
          showError(errorMsg);
          setOtpError(true);
        }
      } else {
      ...
      if (error.response?.status === 400) {
        errorMessage = error.response?.data?.message || error.response?.data?.error || "Invalid data. Please check your inputs.";
  ...
  const handleOtpResend = async () => {
    try {
      ...
      if (pendingProfileData && pendingProfileData.contact) {
        const pendingDigits = pendingProfileData.contact.replace(/\D/g, "");
        mobileNumber = pendingDigits.slice(-10);
      }

      if (!mobileNumber || mobileNumber.length !== 10) {
        showError("Invalid mobile number. Please check your contact number.");
        return;
      }

      const response = await accountService.resendOTP(mobileNumber, countryCode);
      ...
      if (response.success && response.code === 200) {
        ...
        const errorMsg = response.message || response.error || "Failed to resend OTP. Please try again.";
        showError(errorMsg);
      }
    } catch (error) {
  ...
  const handlePasswordSave = (passwords) => {
    // console.log("Password save:", passwords);
    setShowPasswordModal(false);
    onSave(); // Save all changes after password confirmation
  };
  ...
  const hasChanges = editedFields.size > 0 || (editingField && tempValue !== currentFields[editingField]);
  // console.log("Has changes:", hasChanges, "Edited fields size:", editedFields.size, "Currently editing:", editingField);

  return (
    <>
      <Modal open={open && !showOtpModal && !showPasswordModal} onClose={onClose}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <MobileNumberInput
                    ...
                    onChange={(value) => {
                      setCurrentFields(prev => ({ ...prev, contact: value }));
                      setEditedFields(prev => new Set([...prev, "contact"]));
                    }}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FieldRow
                  ...
                  isEdited={editedFields.has("password")}
    ...
    <div className="flex items-center justify-between">
      <div className="flex w-full justify-between items-center">
        {isEditing ? (
          ...
          <>
            <span className="w-full px-3 py-4 text-[var(--color-neutral-secondary)] pl-4 break-all">{value}</span>
            <button
              className="cursor-pointer flex items-center ml-2"
              onClick={onEdit}
              aria-label={`Edit ${label}`}
            >
              {isEdited ? <MdDone className="w-5 h-5 text-[var(--info-panel-view-bg)]" /> : <PencilLine className="w-5 h-5 text-[var(--info-panel-view-bg)]" />}
            </button>
          </>
        )}
      </div>
