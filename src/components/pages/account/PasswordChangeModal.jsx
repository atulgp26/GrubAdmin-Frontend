...
export default function PasswordChangeModal({
  ...
  onBack,
  onSave,
  isAddMode = false, // true = no current password field
}) {
  ...
  const isFormValid = isAddMode
    ? passwords.new && passwords.confirm && passwords.new === passwords.confirm
    : passwords.current && passwords.new && passwords.confirm && passwords.new === passwords.confirm;
...
function PasswordField({ placeholder, value, onChange, showPassword, onToggleVisibility }) {
  return (
    <div className="relative">
      ...
      <button
        type="button"
        className="absolute right-3 top-1/2 transform -translate-y-1/2 text-[var(--color-brand-default)]"
