...
const AddCategory = ({ open, onClose, mode = "add", initialValues = null }) => {
		...
		if (typeof value === "function") {
			setSelectedRole((prev) => {
				...
				return Array.isArray(next)
					? next
							.map((item) => normalizeVerticalId(item))
							.filter((id) => id !== "")
					: [];
			});
			return;
	...
	useEffect(() => {
		const nameValid =
			categoryName.trim().length > 0 && nameCharCount <= MAX_NAME_CHARS;
		...
		const loadIcons = async () => {
			try {
				...
				if (configRes?.success && configRes?.code === 200) {
					const baseCfg = (configRes.data?.configs || []).find(
	...
	const hasChanges = React.useMemo(() => {
		...
		const initialIcon =
			initialValues?.icon ??
			(iconOptions && iconOptions.length ? iconOptions[0].value : "");
	...
	const handleConfirm = async () => {
		const enabled =
			mode === "edit" ? isFormValid && hasChanges : isFormValid;
		if (!enabled) return;
		...
		try {
			...
			if (mode === "edit" && initialValues?.id) {
				res = await faqService.updateCategory({
					id: initialValues.id,
					...payload,
				});
			} else {
			...
			if (res?.success && res?.code === 200) {
				...
				const msg =
					/too long/i.test(rawMsg) && /name/i.test(rawMsg)
			...
			const serverMsg =
				...
				"";
			const msg =
				/too long/i.test(serverMsg) && /name/i.test(serverMsg)
					? "Category name is too long. Please use a shorter name."
					: serverMsg ||
						(mode === "edit"
							? "Failed to update category"
