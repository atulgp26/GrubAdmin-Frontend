...
const EditEmployeeModal = ({ open, onClose, employeeData, onConfirm }) => {
	...
	useEffect(() => {
		if (employeeData && employeeData.originalData) {
			...
			const nameParts = employeeData.name
				? employeeData.name.split(" ")
				: ["", ""];
			...
			const initialFormData = {
				...
				employeeId: employeeData.originalData?.employee_id
					? employeeData.originalData.employee_id
					: employeeData.empId
						? employeeData.empId.replace("#", "")
						: "",
				joiningDate: (() => {
					if (!employeeData.joinDate) return "";
					try {
						const date = new Date(employeeData.joinDate);
			...
			if (initialFormData.phoneNumber) {
				let phoneDigits = initialFormData.phoneNumber.replace(
					/\D/g,
					"",
				);
				...
				if (phoneDigits.startsWith("91") && phoneDigits.length > 2) {
					...
					const withoutCode = initialFormData.phoneNumber
						.substring(3)
						.trim();
	...
	const isFormValid = () => {
		...
		return !firstNameError && !lastNameError && !emailError && !roleError;
	};
	...
	const handleSubmit = () => {
		...
		if (firstNameError || lastNameError || emailError || roleError) {
			...
			showError(
				`Please fill all required fields: ${errorMessages.join(", ")}`,
			);
			return;
		}
