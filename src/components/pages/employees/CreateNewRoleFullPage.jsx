...
const CreateNewRoleFullPage = ({
  ...
  const getVerticalHoverStates = (verticalId) => {
    ...
    if (id === "delivery") {
      return {
        hoverState: "border border-[var(--color-brand-primary-btn)] hover:!border-[var(--color-filter-text)] active:!shadow-[0_0_0_2px_var(--color-shadow-select)]",
        checkedHoverState: "border border-none hover:!bg-[var(--color-filter-text)] hover:!border-[var(--color-filter-text)] active:!bg-[var(--color-brand-primary-btn)] active:!border-[var(--color-brand-primary-btn)] active:!shadow-[0px_0px_0px_2px_var(--color-shadow-select)]"
      };
    }
    return { hoverState: "", checkedHoverState: "" };
  };

  const CustomCheckbox = ({ checked, onChange, colorVar, hoverState, checkedHoverState }) => {
  ...
  const handleSaveRole = async () => {
    console.log("handleSaveRole called", { searchValue, selected, permissions, editRole });
    ...
    if (!isSuperAdmin && getVerticalCounts().used === 0) {
      ...
      if (verticalsSectionRef?.current) {
        try {
          verticalsSectionRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
        } catch (_) {}
      }
    ...
    try {
      ...
      if (!isSuperAdmin) {
        ...
        selected.forEach((moduleName) => {
          ...
          const modulePermissionsList = modulePermissions[moduleName] || [];
          ...
          const checkedPermissions = modulePermissionsList
            .filter((perm) => {
              const isChecked = modulePermissionsObj[perm.id] === true;
              return isChecked;
            })
            .map((perm) => perm.label); // Use the original label from API exactly as it comes

          console.log(`Checked permissions for ${moduleName}:`, checkedPermissions);

          // Only add module if it has permissions
          if (checkedPermissions.length > 0) {
            // Use original module key from API (stored in _moduleKeyMap) - this is critical!
            const moduleKey = moduleKeyMap[moduleName] || 
                             moduleName.charAt(0).toLowerCase() + moduleName.slice(1);
            permissionsData[moduleKey] = checkedPermissions;
            console.log(`Added module "${moduleName}" with API key "${moduleKey}":`, checkedPermissions);
          }
        });
      }

      // Add selected verticals inside permissions object with their names when not super admin
      if (!isSuperAdmin) {
        const selectedVerticalNames = Object.entries(permissions.verticals || {})
          .filter(([, isChecked]) => !!isChecked)
          .map(([id]) => {
            // Find the vertical name from verticalsData
        ...
        if (selectedVerticalNames.length > 0) {
          permissionsData.verticals = selectedVerticalNames;
        }
      }
      ...
      const roleIdString = roleIdForUpdate ? String(roleIdForUpdate).trim() : null;
      
      console.log("Calling API - Edit mode:", !!editRole, "Role ID:", roleIdString);
      ...
      console.log("Form values:", { id: editRole?.id, name: editRole?.name });
      ...
      const response = editRole && roleIdString
      ...
      if (response.success && response.code === 200) {
        ...
        showSuccess("Success", `${searchValue.trim()} role has been ${action}.`);
        // Calculate new permissions count for summary
        const permissionsCount = Object.values(permissionsData || {}).reduce((acc, arr) => acc + (Array.isArray(arr) ? arr.length : 0), 0);
        onSave({
        ...
        const errorMsg = response.error || response.message || `Failed to ${editRole ? 'update' : 'create'} role.`;
        showError(errorMsg);
      }
    } catch (error) {
      console.error(`Error ${editRole ? 'updating' : 'creating'} role:`, error);
      ...
      try {
        ...
        if (permissionsResponse.success && permissionsResponse.data?.permissions) {
          ...
          Object.keys(permissionsData).forEach((moduleKey) => {
            ...
            transformedPermissions[moduleName] = permissionList.map((permission, index) => ({
              id: toId(permission),
              label: permission,
            ...
            transformedPermissions._moduleKeyMap[moduleName] = moduleKey;
          });
        ...
        if (verticalsResponse.success && verticalsResponse.code === 200) {
          ...
          const transformedVerticals = apiVerticals.map((v) => {
            const id = (v?.name || v?.id || v).toString().toLowerCase();
  ...
  useEffect(() => {
    if (open && editRole && !loading && verticalsData.length > 0) {
      ...
      setIsSuperAdmin(editRole.originalData?.is_super_admin === true || false);

      // Populate modules and permissions from permissions_json
      const rolePermissionsJson = editRole.originalData?.permissions_json || {};
      ...
      Object.keys(rolePermissionsJson).forEach((moduleKey) => {
        ...
        if (Array.isArray(permissionLabels) && permissionLabels.length > 0) {
      ...
      const normalizedRoleVerticals = Array.isArray(roleVerticals)
        ? roleVerticals.map(v => {
          ...
          return (v?.id || v?.name || v)?.toString().toLowerCase().trim();
        })
        : [];

      console.log('Edit role normalized vertical IDs:', normalizedRoleVerticals);
      ...
      if (normalizedRoleVerticals.length > 0) {
        ...
        normalizedRoleVerticals.forEach((vNameFromAPI) => {
          ...
          const verticalKey = matchingVertical ? matchingVertical.id : vNameFromAPI;
        ...
        console.log('Setting verticals in permissionsState:', permissionsState.verticals);
      }
    ...
    return (
      <FullPageModal open={open} onClose={onClose}>
        <div className="h-screen bg-white flex items-center justify-center">
          <div className="text-[var(--color-neutral-secondary)]">Loading...</div>
        </div>
      </FullPageModal>
    );
  }
                    ...
                    <div className="mb-6">
                      ...
                      {isSuperAdmin && (
                        <p className="text-[var(--color-stroke-brand)] text-sm mt-1 ml-7">
                          Super Admin has access to all permissions and modules
                        </p>
                      )}
                    </div>
                      ...
                      >
                        <div ref={verticalsSectionRef}>
                          ...
                          {verticalsData.map((v) => {
                            ...
                            const normalizedVId = v.id?.toLowerCase().trim();
                            const normalizedVLabel = v.label?.toLowerCase().trim();
                            const isChecked = permissions.verticals?.[normalizedVId] || 
                                            permissions.verticals?.[normalizedVLabel] ||
                                            false;
                            ...
                            <div key={v.id} className="flex items-center px-6 border-b gap-2">
                              <CustomCheckbox
                                ...
                                colorVar={v.color}
                                hoverState={v.hoverState}
                                checkedHoverState={v.checkedHoverState}
                              />
