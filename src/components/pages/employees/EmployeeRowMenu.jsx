	...
	return (
		...
		>
			{canEdit && (
				...
				>
					<LuPencilLine className="mr-2 w-5 h-5 text-[var(--color-neutral-light)] " />{" "}
					Edit employee details
				</Button>
			)}
			{groupId !== "unassigned" && canViewLogs && (
				<Link
					href={`/employees/activelogs?id=${employeeId}`}
					className="block"
				>
					<Button
						variant="profile"
			...
			{canSuspend && (
				<Button
					...
					className="flex btn-size-md-sm !rounded-none items-center w-full px-4 py-3 text-sm text-[var(--color-neutral-secondary)] active:text-[var(--color-neutral-primary)]"
					onClick={() => {
						onRowAction("suspend", employeeId);
						onClose();
					}}
				>
					<RxCrossCircled className="mr-2 w-5 h-5 text-[var(--color-neutral-light)] " />{" "}
					Suspend employee
				</Button>
			)}
			{canDelete && (
				<Button
					...
					onClick={() => {
						onRowAction("delete", employeeId);
						onClose();
					}}
				>
					<FiTrash2 className="mr-2 w-5 h-5 text-[var(--notif-error)] group-hover:text-[var(--info-panel-view-bg)] group-active:text-[var(--color-filter-text)]" />{" "}
					Delete employee
				</Button>
			)}
		</div>
