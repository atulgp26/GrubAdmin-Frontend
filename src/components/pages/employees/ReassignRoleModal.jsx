...
export default function ReassignRoleModal({
        ...
        try {
          ...
          if (
            ...
            adminsResponse.data.admins.forEach((admin) => {
              const roleId = (admin?.role?.id) || admin?.role_id || admin?.roleId || admin?.roleID;
              if (!roleId) return;
              const key = String(roleId);
              countsByRoleId[key] = (countsByRoleId[key] || 0) + 1;
            });
          }

          if (rolesResponse.success && rolesResponse.code === 200 && rolesResponse.data?.roles) {
            const rolesData = rolesResponse.data.roles.map((role, index) => {
              const permissionsCount = calculatePermissionsCount(role.permissions_json);
              const roleId = role.id ?? `role-${index}`;
              const assignmentCount = countsByRoleId[String(roleId)] || 0;
              return {
                id: roleId,
                name: role.name,
  ...
  return (
    <Modal open={open} onClose={handleClose} width="w-[920px]" height="h-auto max-h-full">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-1 text-lg text-[var(--color-neutral-secondary)]">
              <CheckBox checked={hideRole} onChange={(e) => setHideRole(e.target.checked)} />
        ...
        <div className="flex-1 rounded-lg">
          ...
          <div className="flex-1 overflow-y-auto max-h-[400px]">
            {loading ? (
              ...
              <div className="flex items-center justify-center py-12">
                <div className="text-[var(--color-stroke-brand)]">No roles found.</div>
              </div>
            ) : (
