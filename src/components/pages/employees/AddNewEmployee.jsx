...
const dateRef = useRef(null);
  ...
  const handleRoleChange = (newSelectedRoles) => {
    ...
    const roleValue = newSelectedRoles.length > 0 ? newSelectedRoles[0] : '';
    setFormData(prev => ({
      ...prev,
      role: roleValue
    }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const selectedRoleOption = roleOptions.find(role => role.value === formData.role);
      if (selectedRoleOption) {
        roleId = selectedRoleOption.value;
      } else {
        // Fallback: try to find by label (in case it was stored as label)
        const roleByLabel = roleOptions.find(role => role.label === formData.role);
        if (roleByLabel) {
          roleId = roleByLabel.value;
        } else {
          console.warn("Role not found in roleOptions. formData.role:", formData.role);
        }
      }
      ...
      if (firstNameError || emailError || roleError || phoneError) {
        ...
        showError(phoneError || 'Please fix the validation errors before submitting');
        setLoading(false);
        return;
      }
      ...
      if (formData.employeeId && formData.employeeId.trim()) {
        payload.employee_id = formData.employeeId.trim().replace(/^#\s*/, '');
      }
      ...
      if (response.success && response.code === 200) {
        ...
        showSuccess('Success!', `${fullName || 'Employee'} added successfully. Their role can be managed anytime from their profile.`);
        handleReset();
        onClose();
        if (onConfirm) {
          onConfirm(formData);
        ...
        let errorMsg = response.error || response.message || "Failed to create employee. Please try again.";
        const lower = (errorMsg || "").toLowerCase();
        if (lower.includes('unique') || lower.includes('collision')) {
          errorMsg = "This email, mobile number, or Employee ID already exists. Please use unique values.";
        }
        showError(errorMsg);
      ...
      if (errorMessage.toLowerCase().includes("unique") || errorMessage.toLowerCase().includes("collision")) {
        errorMessage = "This email, mobile number, or Employee ID already exists. Please use unique values.";
      }
    ...
    >
      <div className="space-y-6">
        ...
        <div className="space-y-2 mt-2">
          <h1 className='text-[var(--color-neutral-primary)] font-semibold text-2xl'>Add new employee</h1>
            ...
            <div className="relative">
              <MobileNumberInput
                ...
                onChange={(phoneValue) => {
                  ...
                  setValidationErrors(prev => ({
                    ...prev,
                    phoneNumber: digits.length > 0 && digits.length < 10 ? 'Mobile number must be 10 digits' : ''
                  }));
                }}
                ...
                onValidationChange={(isValid) => {
                  ...
                  setValidationErrors(prev => ({
                    ...prev,
                    phoneNumber: digits.length > 0 && !isValid ? 'Mobile number must be 10 digits' : ''
                  }));
                }}
              />
              {validationErrors.phoneNumber && (
                <p className="absolute top-full left-0 mt-1 text-xs text-red-500">{validationErrors.phoneNumber}</p>
              )}
            </div>
            <div className="relative">
              <Input
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <h3 className="text-[var(--color-neutral-secondary)] text-base font-medium">
              Employee ID <span className="text-sm text-[var(--color-neutral-secondary)]">(optional)</span>
            </h3>
            <div className="relative">
              <Input
                type="text"
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            <div className="relative">
              ...
              {validationErrors.location && (
                <p className="absolute top-full left-0 mt-1 text-xs text-red-500">{validationErrors.location}</p>
              )}
            </div>
          </div>
          <div className="space-y-2">
        ...
        <div className="flex gap-4 pt-6 border-t border-[var(--color-box-border)]">
          ...
          >
            <span className="flex items-center justify-center gap-2">
              {isFormValid() && !loading && <MdDone stroke={1} className='w-6 h-6'/> }
