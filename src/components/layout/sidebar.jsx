...
const visibleNavItems = React.useMemo(() => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed left-0 top-0 h-full border-r border-[var(--color-stroke-neutral)] transition-all duration-300 z-40 ${collapsed ? "-translate-x-full" : "translate-x-0"
          } w-60 hidden md:block`}
      >
        <div className="flex flex-col h-full">
          <div className="p-6">
            <Image src="/GrubPac_admin.svg" alt="GrubPac" width={124} height={15} />
          </div>
          <nav className="flex-1 px-4">
            <div className="space-y-1">
              {visibleNavItems.map((item) => (
                <React.Fragment key={item.id}>
                  {item.id === "support" && <hr className="my-2 border-[var(--color-stroke-neutral)]" />}
      ...
      <div
        className={`fixed left-0 top-0 h-full bg-[var(--color-neutral-secondary-bg)] transition-all duration-300 z-40 ${collapsed ? "-translate-x-full" : "translate-x-0"
          } w-64 md:hidden`}
      >
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between p-6 border-b border-[var(--color-stroke-neutral)]">
            <h1 className="text-2xl font-bold text-[var(--color-brand-default)]">GrubPac</h1>
            <button
              onClick={onClose}
