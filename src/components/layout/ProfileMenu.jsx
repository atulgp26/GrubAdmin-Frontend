...
export default function ProfileMenu() {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: <IoIosWarning className="w-6 h-6 text-[var(--color-neutral-light)] group-hover:text-[var(--info-panel-view-bg)] group-active:text-[var(--color-filter-text)]" />,
    },
    {
      label: "Log out",
      icon: <Icon strokeWidth={2} name="logout" className=" text-[var(--color-neutral-light)] group-hover:text-[var(--info-panel-view-bg)] group-active:text-[var(--color-filter-text)]" />,
    },
  ];
  ...
  return (
    <div className="relative flex flex-col items-center" ref={ref}>
      ...
      <div
        className={`absolute bottom-18 left-0 w-56 rounded-lg shadow-[4px_4px_8px_0px_var(--color-notif-shadow-soft),0px_0px_4px_0px_var(--color-notif-shadow-strong)] bg-white border border-[var(--color-stroke-neutral)] divide-y divide-[var(--color-stroke-neutral)] overflow-hidden transition-all duration-200 z-50 ${open
      ...
      >
        <Link href="/account" className="block" passHref legacyBehavior>
          ...
          >
            ...
            <span className="text-sm font-normal text-[var(--color-neutral-primary)]">{menuItems[0].label}</span>
          </Button>
        </Link>
        {menuItems.slice(1).map((item, i) => {
          ...
          if (item.label === "Transfer ownership") {
            return (
              <Link key={item.label} href="/transfer-ownership" className="block" passHref legacyBehavior>
                <Button
                  variant="profile"
                ...
                >
                  ...
                  <span className="text-sm font-normal text-[var(--color-neutral-primary)]">{item.label}</span>
                </Button>
              </Link>
            );
          }
