...
const ManageRoles = () => {
	...
	const [hideRolesWithAssignment, setHideRolesWithAssignment] =
		useState(false);
	...
	const [selectedEmployeesForReassign, setSelectedEmployeesForReassign] =
		useState([]);
	...
	const loadRolesWithAssignments = async (params = {}) => {
		try {
			...
			if (
				...
				const transformedRoles = rolesResponse.data.roles.map(
					(role, index) => {
						const roleId = role.id || `role-${index}`;
						return {
							id: roleId,
							name: role.name || "Unnamed Role",
							permissions: calculatePermissionsCount(
				...
				if (typeof apiTotal === "number" && apiTotal >= 0) {
					...
					console.warn(
						"API did not return total count for roles, using current page length as fallback",
					);
	...
	const searchSuggestions =
		roles.length > 0
			? Array.from(new Set(roles.map((role) => role.name))).map(
					(name) => ({
	...
	const handleSave = async (summary) => {
		if (roleDescription && roleDescription.trim() !== "") {
			...
			try {
				...
				const oldPermissionsCount = calculatePermissionsCount(
					editingRole?.originalData?.permissions_json || {},
				);
				const newPermissionsCount =
					summary?.permissionsCount ?? oldPermissionsCount;
				const oldRoleName =
					editingRole?.name ||
					editingRole?.originalData?.name ||
					"Role";
				const newRoleName = summary?.roleName || oldRoleName;

				// Optimistically update role details in list immediately
						...
						prev.map((r) =>
							r.id === roleId
								? {
										...r,
										name: newRoleName,
										permissions: newPermissionsCount,
										updated: "Today",
										originalData: {
											...r.originalData,
											name: newRoleName,
				...
				setConfirmRolesData([
					{
						name: `${oldRoleName}`,
						count: assignedCount,
						permissions: oldPermissionsCount,
						type: "old",
					},
					{
						name: `${newRoleName}`,
				...
				setConfirmDescription(
					`This change will immediately affect all ${assignedCount} employees currently assigned to this role.`,
				);
			} catch (e) {
				// Fallback
				setConfirmRolesData([]);
			...
			try {
				...
				console.error("Error refreshing roles after edit:", error);
			}
		} else {
	...
	const handlePermissions = async (role) => {
		...
		const rolePermissionsJson =
			latestRole?.originalData?.permissions_json ||
			role?.originalData?.permissions_json ||
			{};
		...
		try {
			...
			if (
				...
				const selectedCount =
					calculatePermissionsCount(rolePermissionsJson);

				const transformedOptions = Object.keys(permissionsData).map(
					(moduleKey) => {
						const moduleName =
						...
						const roleModulePermissions =
							rolePermissionsJson[moduleKey] || [];

						return {
							group: moduleKey,
							title: moduleName,
							items: permissionsData[moduleKey].map(
								(permission) => {
									// Check if this permission is selected in the role
									const isChecked =
										Array.isArray(roleModulePermissions) &&
										roleModulePermissions.includes(
											permission,
										);
			...
			if (verticalsResponse.success && verticalsResponse.code === 200) {
				...
				const verticalsDataForModal = apiVerticals.map((v) => {
					...
					const isChecked =
						roleVerticalIds.includes(normalizedId) ||
	...
	const handleOpenManagers = async (role) => {
		try {
			...
			if (rows.length >= 0) {
				setRoles((prev) =>
					prev.map((r) =>
						r.id === roleId ? { ...r, assignment: rows.length } : r,
					),
				);
			}
		} catch (error) {
	...
	const handleDelete = () => {
		...
		const firstRoleName =
			selectedCount > 0
				? filteredRoles.find((role) => selectedRoles.has(role.id))
						?.name || "selected roles"
				: "selected roles";
		...
		if (selectedCount === 0) {
			...
			setDescription(
				"This role currently has employees assigned. Reassign them to a new role before deletion.",
			);
		} else {
			setTitle(
				`Delete "${firstRoleName}" and ${otherCount} other roles?`,
			);
			setDescription(
				"Some of the selected roles have employees assigned. Reassign them to new roles before deletion. Roles without assignees will be deleted immediately.",
			);
		}
		setDeleteRole(true);
	...
	const handleDeleteRole = async () => {
		try {
			...
			if (successful.length > 0) {
				showSuccess(
					"Success",
					`${successful.length} role${successful.length !== 1 ? "s" : ""} have been deleted.`,
				);
			...
			if (failed.length > 0) {
				...
				showError(
					`${failed.length} role${failed.length !== 1 ? "s" : ""} could not be deleted: ${errorMessages.join(", ")}`,
				);
			}
	...
	const onDeleteRole = (roleId) => {
		...
		setDescription(
			`This role currently has ${role?.assignment || 0} employees assigned. Reassign them to a new role before deletion.`,
		);
	...
	const handleConfirmRoleAssign = async (selectedRole) => {
		try {
			if (
				!selectedRole?.id ||
				!Array.isArray(selectedEmployeesForReassign) ||
			...
			const response = await employeeService.bulkAssignRole(
				selectedRole.id,
				selectedEmployeesForReassign,
			);
			if (response?.success && response?.code === 200) {
				showSuccess(
					"Success",
					`${selectedEmployeesForReassign.length} employee(s) reassigned to ${selectedRole.name}.`,
				);
	...
	return (
				...
				<div className="flex items-start gap-4">
					...
					<div>
						...
						<p className="text-base text-[var(--color-stroke-brand)] ">
							Roles define what employees can and cannot do inside
							the platform.
						</p>
					</div>
				</div>
				...
				<div className="flex items-center gap-4">
					...
					<label className="flex items-center gap-2 text-lg text-[var(--color-neutral-secondary)]">
						<CheckBox
							...
							onChange={(e) =>
								setHideRolesWithAssignment(e.target.checked)
							}
						/>
						...
						<TableBody>
							{filteredRoles.length === 0 ? (
								<TableRow>
									<TableCell
										colSpan={6}
										className="px-3 py-8 text-center text-[var(--color-stroke-brand)]"
									>
										No roles found.
									</TableCell>
								</TableRow>
											...
											>
												...
												>
													<Icon
														...
														className={`w-5 h-5 ${role.assignment > 0 ? "text-[var(--info-panel-view-bg)]" : "text-[var(--color-neutral-light)] group-hover:text-[var(--color-brand-default)]"} transition-colors`}
													/>
													<span className="text-[var(--color-neutral-secondary)] text-sm font-normal leading-none">
														{role.assignment}
													</span>
												</button>
											</CustomTooltip>
															...
															<div className="w-52 bg-white border border-[var(--color-stroke-neutral)] divide-y divide-[var(--color-stroke-neutral)] rounded-lg shadow-[4px_4px_8px_0_var(--color-notif-shadow-soft),0px_0px_4px_0_var(--color-notif-shadow-strong)]">
																{canEditRoles && (
																	<button
																		...
																		className={`w-full text-left px-4 py-2 flex items-center gap-2 text-[var(--color-neutral-secondary)] text-sm hover:bg-[var(--color-neutral-secondary-bg)] transition-colors`}
																	>
      ...
      /> */}
			...
			<ExportListModal
				...
				onConfirm={async ({ checked }) => {
					try {
						...
						// Add verticals inside permissions object with their IDs
						const selectedVerticalIds = (midLevelData || [])
							.filter((v) => checked[v.id])
							.map((v) => v.id);
						if (selectedVerticalIds.length > 0) {
							permissionsPayload.verticals = selectedVerticalIds;
						}
						const roleId =
							selectedRoleForPermissions?.originalData?.id ||
						...
						const response = await roleService.updateRole(
							roleIdString,
							{
								...
								is_super_admin:
									selectedRoleForPermissions?.originalData
						...
						if (response?.success && response?.code === 200) {
									...
									if (r.id === roleId) {
										return {
											...
											originalData: {
												...(r.originalData || {}),
												permissions_json:
													permissionsPayload, // Update permissions_json in the role
											},
										};
									}
									return r;
							...
							try {
								...
								console.error(
									"Error refreshing roles after update:",
									error,
								);
							}
						...
						console.error(
							"Failed to update role from permissions modal:",
							e,
						);
						showError(
			...
			<ReassignRoleModal
				...
				onConfirm={
					isDirectReassign ? handleConfirmRoleAssign : handleConfirm
				}
				title={
					isDirectReassign
						? `Reassign role to ${selectedEmployeesForReassign.length} employees`
