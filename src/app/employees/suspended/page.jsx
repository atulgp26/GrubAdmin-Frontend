...
const SuspendedEmployees = () => {
	...
	const canActivateEmployees =
		can("active employees", "employees") || can("active employees");
			...
			if (
				...
				const transformed = response.data.admins.map((admin, index) => {
					...
					return {
						...
						role: roleName,
						suspended: formatDate(
							admin.suspended_at || admin.updated_at,
						),
						originalData: admin,
					};
				});
				...
				if (selectedEmployeeId) {
					const foundInResponse = transformed.find(
	...
	const handleExportConfirm = async ({ scope, checked }) => {
		try {
			...
			console.log(
				"Current filteredEmployees count:",
			...
			if (response && typeof response === "object" && response.blob) {
				...
				const filename =
					response.filename ||
					`suspended_employees_export_${new Date().toISOString().split("T")[0]}.csv`;
			...
			const errorMessage =
				error.message ||
				"Failed to export suspended employees. Please try again.";
	...
	const filteredEmployees = employeesWithPreserved.filter((employee) => {
		...
		if (selectedRole.length > 0) {
			const employeeRoleId =
				employee.originalData?.role_id ||
				employee.originalData?.role?.id;
		...
		try {
			...
			if (pendingReassignRole) {
				...
				if (idsNeedingReassign.length > 0) {
					try {
						await employeeService.bulkAssignRole(
							pendingReassignRole.id,
							idsNeedingReassign,
						);
					} catch (assignErr) {
						console.error(
							"Bulk assign role before activation failed:",
							assignErr,
						);
					}
				}
			...
			if (res?.success && res.code === 200) {
				...
				const errorMsg =
					res?.error ||
					res?.message ||
					"Failed to reactivate employee(s).";
				showError(errorMsg);
			}
		} catch (e) {
						...
						<TableCell className="w-12 p-4">
							<TableCheckbox
								checked={
									...
									group.items.every((emp) =>
										selectedEmployees.has(emp.id),
									)
								}
								indeterminate={
									group.items.some((emp) =>
										selectedEmployees.has(emp.id),
									) &&
									!group.items.every((emp) =>
										selectedEmployees.has(emp.id),
