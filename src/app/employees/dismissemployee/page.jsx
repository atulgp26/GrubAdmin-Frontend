...
const DismissEmployees = () => {
  ...
  const canViewEmployeeLogs = can('view employee logs', 'employees') || can('view employee logs');
  ...
  const fetchDismissedEmployees = useCallback(async () => {
    try {
      ...
      if (selectedRole.length > 0) {
        ...
        const selectedRoleOption = roleOptions.find(ro => ro.id === selectedRole[0]);
      ...
      if (response?.success && response.code === 200 && response.data?.admins) {
        const transformed = response.data.admins.map((admin, index) => {
          ...
          return {
            ...
            role: roleName,
            dismissed: formatDate(admin.dismissed_at || admin.updated_at),
            originalData: admin,
          };
        });
  ...
  const exportOptions = [
    {
      ...
      title: "Scope",
      items: [
        { id: "employees", label: "All dismissed employees", type: "radio" },
        {
          id: "filteredList",
  ...
  const handleExportConfirm = async ({ scope, checked }) => {
    try {
      ...
      if (response && typeof response === 'object' && response.blob) {
        ...
        const filename = response.filename || `dismissed_employees_export_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const filteredEmployees = employees.filter((employee) => {
    if (selectedRole.length > 0) {
      ...
      if (selectedRoleOption) {
        ...
        if (employeeRoleName.toLowerCase() !== selectedRoleOption.label.toLowerCase()) {
          return false;
        }
      }
    }
  ...
  const handleSelectEmployee = (empId, checked) => {
    ...
    setSelectAll(newSelected.size === filteredEmployees.length && filteredEmployees.length > 0);
  };
    ...
    <div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <div className="text-[var(--color-neutral-secondary)]">Loading dismissed employees...</div>
        </div>
      ) : groupByRole ? (
        <GroupCollapseTable
