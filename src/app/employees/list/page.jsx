...
const EmployeesList = () => {
  ...
  const canViewActive = can("view active employees", "employees") || can("view active employees");
  const canViewSuspended = can("view suspended employees", "employees") || can("view suspended employees");
  const canAddEmployees = can("add employees", "employees") || can("add employees");
  const canExportEmployees = can("export employees", "employees") || can("export employees");
  const canSuspendEmployees = can("suspend employees", "employees") || can("suspend employees");
  ...
  const canViewDismissed = can("view dismissed employees", "employees") || can("view dismissed employees");
  const hasAnyActions = canAddEmployees || canViewRoles || canViewDismissed || canExportEmployees;
  // For actions
  const actionOptions = [
    {
      group: "Dashboard",
  ...
  const [selectedRoleForReassign, setSelectedRoleForReassign] = useState(null); // Store selected role from ReassignRoleModal
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      ...
      if (response.success && response.code === 200 && response.data?.admins) {
        ...
        const transformedEmployees = response.data.admins.map((admin, index) => {
        ...
        if (selectedEmployeeId) {
          const foundInResponse = transformedEmployees.find(emp => emp.id === selectedEmployeeId);
          if (!foundInResponse) {
            ...
            setEmployees((prevEmployees) => {
              ...
              if (preservedFromPrev) {
                ...
                return [preservedFromPrev, ...transformedEmployees];
              }
        ...
        if (typeof apiTotal === 'number' && apiTotal >= 0) {
          ...
          console.warn('API did not return total count, using current page length as fallback');
  ...
  const filteredEmployeesWithPreserved = employeesWithPreserved.filter((employee) => {
    // If a specific employee was selected from suggestions, prioritize showing that employee
    if (selectedEmployeeId) {
      return employee.id === selectedEmployeeId;
    }
  ...
  const visibleFlatEmployees = searchValue && searchValue.trim()
  ...
  const totalItems = searchValue && searchValue.trim()
  ...
  const handleSuspend = async () => {
    try {
      ...
      const adminIds = selectedEmployees.size > 0
      ...
      if (response.success && response.code === 200) {
        ...
        const errorMsg = response.error || response.message || "Failed to suspend employee(s). Please try again.";
        showError(errorMsg);
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (res?.success && res.code === 200) {
        ...
        const errorMsg = res?.error || res?.message || "Failed to delete employees.";
        showError(errorMsg);
      }
    } catch (e) {
  ...
  const handleExportConfirm = async ({ scope, checked }) => {
    try {
      ...
      if (response && typeof response === 'object' && response.blob) {
        ...
        const filename = response.filename || `employees_export_${new Date().toISOString().split('T')[0]}.csv`;
      ...
      const errorMessage = error.message || "Failed to export employees. Please try again.";
  ...
  const handleConfirmChanges = async () => {
    ...
    try {
      ...
      if (response.success && response.code === 200) {
        ...
        const errorMsg = response.error || response.message || "Failed to assign role. Please try again.";
        showError(errorMsg);
      }
    } catch (error) {
    ...
    try {
      ...
      if (error.response?.data) {
        ...
        if (errorData.errors) {
          ...
          if (errorMessage.toLowerCase().includes('last_name') || errorMessage.toLowerCase().includes('last name')) {
  ...
  const groupEmployeesByRole = () => {
    ...
    const pushGroup = (roleLabel, roleId) => {
      const roleEmployees = employees.filter((emp) => {
        const rid = emp?.originalData?.role?.id || emp?.originalData?.role_id;
          ...
          <CustomTooltip
            title={
              <div className="space-y-2">
                <div className="text-[var(--color-stroke-brand)] text-sm">{permissionsCount} permissions</div>
                <div
            ...
            arrowPosition="left"
          >
            <span className="cursor-default hover:underline text-[var(--color-stroke-brand)] font-medium text-sm">{String(roleLabel || '').toUpperCase()}</span>
          </CustomTooltip>
        ),
        items: roleEmployees,
      });
  ...
  const renderGroupTable = (group) => {
    ...
    const allGroupSelected = groupEmployeeIds.length > 0 && groupEmployeeIds.every(id => selectedEmployees.has(id));
    const someGroupSelected = groupEmployeeIds.some(id => selectedEmployees.has(id));
    ...
    const handleGroupSelectAll = (checked) => {
      ...
      setSelectedEmployees(newSelected);
      setSelectAll(newSelected.size === employees.length && employees.length > 0);
    };
                ...
                <TableCell className="w-12 p-4">
                  ...
                  >
                    <EmployeeRowMenu
                      open={menuOpen === employee.id}
                      onClose={() => setMenuOpen(null)}
                      onRowAction={(action) => handleRowAction(action, employee.id)}
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          <span className="text-sm text-[var(--color-stroke-brand)]">
            Showing {visibleFlatEmployees.length} of {searchValue && searchValue.trim() ? filteredEmployeesWithPreserved.length : totalCount}
          </span>
          <div className="w-48">
            <MultiSelectDropdown
              options={roleOptions}
        ...
        <>
          ...
          {selectedEmployees.size > 0 && (
            <TableActionBar
              ...
              onSuspend={canSuspendEmployees ? () => setSuspendEmployeeModal(true) : undefined}
              onDelete={canDeleteEmployees ? () => setDeleteEmployeeModal(true) : undefined}
      ...
      {hasAnyActions && (
        ...
        >
          <div className="">
            {canAddEmployees && (
              ...
              >
                <div className="h-full flex items-center items-center justify-center">
                  <Icon name="user_plus" className="w-5 h-5 text-[var(--notif-success)]" />
                </div>
                <div className=" flex-1">
                  ...
                  <p className="text-xs text-[var(--color-stroke-brand)] leading-relaxed">
                    Add employees to manage GrubPac operations. <br />
                    Assign roles like Admin, Support, or Technician to give <br />
              ...
              <Link href="/employees/roles">
                ...
                >
                  <div className="flex justify-center">
                    <Icon name="notes_info" className="w-5 h-5 text-[var(--color-neutral-light)]" />
                  </div>
                  <div className="flex-1">
                    <h3 className="text-sm text-[var(--color-neutral-secondary)] mb-1">
                      Manage roles
                    ...
                    <p className="text-xs text-[var(--color-stroke-brand)] leading-relaxed">
                      Roles define what employees can and cannot do inside <br />
                      the platform. <br />
                      Use predefined roles for quick setup or create custom <br />
                      ones tailored to your organization's needs.
                    </p>
                  </div>
                </div>
              </Link>
            ...
            {canViewDismissed && (
              <Link href="/employees/dismissemployee">
                <div className="cursor-pointer border-b border-[var(--color-stroke-neutral)] hover:bg-[var(--color-neutral-secondary-bg)] flex items-center gap-3 px-4 py-3">
                  <div className="flex justify-center">
                    <Icon name="user_wrong" className="w-5 h-5 text-[var(--notif-error)]" />
                  </div>
                  <div className="flex-1">
                    <h3 className="text-sm text-[var(--color-neutral-secondary)] mb-1">
                      Dismissed employees
                    ...
                    <p className="text-xs text-[var(--color-stroke-brand)] leading-relaxed">
                      View records of employees whose accounts were <br /> permanently
                      deleted. <br /> Kept for compliance and historical reference.
                    </p>
                  </div>
                </div>
              </Link>
            ...
            {canExportEmployees && (
              <div onClick={handleTakeAction} className="cursor-pointer hover:bg-[var(--color-neutral-secondary-bg)] flex items-center gap-3 px-4 py-3">
                <div className="flex justify-center">
                  <Icon name="download" className="w-5 h-5 text-[var(--color-neutral-light)]" />
                </div>
                <div className="flex-1">
                  <h3 className="text-sm text-[var(--color-neutral-secondary)] mb-1">
                  ...
                  <p className="text-xs text-[var(--color-stroke-brand)] leading-relaxed">
                    Download your employee records for reporting <br /> or backup.
                  </p>
                </div>
              </div>
            )}
