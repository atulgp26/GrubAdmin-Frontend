...
export default function Categories() {
  ...
  const canViewSupport = can('view active resources', 'support') || can('view active resources');
  const canViewSuspendedCats = can('view suspended categories', 'support') || can('view suspended categories');
  ...
  useEffect(() => {
    ...
    const selectedIds = selectedRole.map((v) => (v?.id !== undefined ? String(v.id) : String(v)));
    if (verticalOptions?.length && selectedIds.length === verticalOptions.length) {
      setVertical('all');
    } else {
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        const [cfg, iconsRes, verticalsRes, catsRes] = await Promise.all([
        ...
        if (cfg?.success && cfg?.code === 200) {
          const baseCfg = (cfg.data?.configs || []).find(c => c.key === "icon_base_url");
          if (baseCfg?.value) setIconBaseUrl(`${baseCfg.value}`.replace(/\/+$/, ''));
        }

        if (iconsRes?.success && iconsRes?.code === 200) {
          const map = {};
          (iconsRes.data?.icons || []).forEach(ic => { map[ic.id] = ic.bucket_key; });
          setIconIdToKey(map);
        }
        ...
        if (catsRes?.success && catsRes?.code === 200) {
          ...
          const normalized = list.map(c => ({
            ...
            title: c.name,
            iconId: c.icon_id,
            verticalId: c.vertical_id !== undefined && c.vertical_id !== null ? String(c.vertical_id) : "",
          ...
          try {
            const questionsPromises = normalized.map(cat =>
              faqService.getFaqsByCategory(cat.id).catch(() => ({ data: { faqs: [] } }))
            );
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const res = await faqService.getCategories({ query: q, include_questions: 'true' });
  ...
  const filtered = useMemo(() => {
    ...
    if (s) {
      ...
      allQuestions.forEach(q => {
        ...
        const answerMatches = (q.answer || "").toLowerCase().includes(s);
      ...
      categoriesToShow = categories.filter((c) => {
        ...
        if (Array.isArray(v)) {
          const selectedSet = new Set(v.map((x) => x?.toString?.().toLowerCase?.() || String(x).toLowerCase()));
          matchesVertical = selectedSet.has(idKey);
        } else {
          const selectedKey = (v || "").toString().toLowerCase();
          matchesVertical = selectedKey === "all" || idKey === selectedKey;
        }
        const matchesCategoryName =
          c.title.toLowerCase().includes(s) ||
          (c.description || "").toLowerCase().includes(s);
        const hasMatchingQuestions = categoriesWithMatchingQuestions.has(c.id);

        return matchesVertical && (matchesCategoryName || hasMatchingQuestions);
      });
    } else {
      categoriesToShow = categories.filter((c) => {
        const v = vertical;
  ...
  const exportOptions = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        title: "Extra details",
        items: [
          { id: "Only published FAQs", label: "Only published FAQs", type: "radio", variant: "gray" },
          { id: "Only draft FAQs", label: "Only draft FAQs", type: "radio", variant: "gray" },
        ],
      },
    ];
  }, [verticalOptions]);
  ...
  const canExport = (
    can("export active resources", "support") || can("export active resources")
  );
  const actionsData = [
    ...(canAdd ? [{
      ...
      onClick: () => { setIsActionModalOpen(false); setAddOpen(true); },
    }] : []),
    ...(canViewSuspendedCats ? [{
      title: "View suspended categories",
      description: "View the categories & FAQs currently suspended.",
      icon: "notes_info",
    ...
    ...(canReorder ? [{
      ...
      description: "Change the display order of active categories.",
      icon: "arrows_up_down",
      iconColor: "text-[var(--color-neutral-light)]",
      onClick: () => {
        const visibleCount = filtered.length;
        if (visibleCount <= 1) {
          const msg = visibleCount === 0 ? "No categories to reorder." : "Only one category available. Reordering requires 2 or more.";
          showError(msg);
        } else {
    ...
    ...(canExport ? [{
      title: "Export all",
      description: "Export all FAQs and categories for reference.",
      icon: "download",
      iconColor: "text-[var(--color-neutral-light)]",
  ...
  const confirmReorder = async (reorderedData) => {
    try {
      ...
      if (response?.success) {
        ...
        const errorMsg = response?.message || response?.error || "Failed to reorder categories. Please try again.";
        showError(errorMsg);
      }
    } catch (error) {
      const errorMsg = error?.response?.data?.message || error?.response?.data?.error || error?.message || "Failed to reorder categories. Please try again.";
      showError(errorMsg);
    }
  }
  ...
  const handleExportConfirm = async ({ scope, checked }) => {
    try {
      ...
      if (scope === "All categories" || !scope) {
        ...
        const matchedOption = verticalOptions.find((opt) => String(opt.id) === verticalKey);
        if (matchedOption) {
      ...
      if (response && typeof response === 'object' && response.blob) {
        ...
        const filename = response.filename || `faqs_export_${new Date().toISOString().split('T')[0]}.csv`;
      ...
      const errorMessage = error.message || "Failed to export FAQs. Please try again.";
    ...
    <div className="min-h-[calc(100vh-150px)]">
      <div className="flex items-center justify-between mb-6">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-semibold text-[var(--color-neutral-primary)]">Client support management</h1>
          <p className="text-[var(--color-stroke-brand)]">Client support management</p>
        </div>
        <div className="flex items-center gap-4">
          {hasAnyActions && (
            <Button variant="primary" className="btn-size-md-sm !px-3 font-medium" onClick={() => setIsActionModalOpen(true)}>TAKE ACTION</Button>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <SearchWithSuggestions
            ...
            onSelect={(item) => {
              if (item.type === "question") {
                router.push(`/support/supportdefaultlogs?category_id=${item.categoryId}&question_id=${item.questionId}`);
              } else {
                setSearch(item.name);
              }
            }}
        ...
        <div className="flex items-center gap-3 ml-auto">
          <div className="text-sm text-[var(--color-stroke-brand)]">Showing {filtered.length} of {categories.length}</div>
          <div className="w-48">
            <MultiSelectDropdown
              ...
              setSelected={(val) => {
                setSelectedRole(val);
                const ids = Array.isArray(val) ? val.map(v => (v?.id !== undefined ? String(v.id) : String(v))) : [];
                if (!ids.length || (verticalOptions?.length && ids.length === verticalOptions.length)) {
                  setVertical('all');
                } else {
                  setVertical(ids);
                }
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
          {filtered.map((cat) => (
            <div key={cat.id} onClick={() => router.push(`/support/supportdefaultlogs?category_id=${cat.id}`)}
              ...
              <div className="relative flex flex-col items-center gap-3 text-center w-full">
                <div className="w-full flex justify-end">
                  ...
                  >
                    <BsThreeDotsVertical strokeWidth={1} className="w-5 h-5 text-[var(--color-stroke-brand)]" />
                  </button>
                  <DropdownPortal
                    targetRef={
                      buttonRefs.current[cat.id]
                        ? { current: buttonRefs.current[cat.id] }
                        : null
                    }
                    open={menuOpen === cat.id}
                    onClose={() => setMenuOpen(null)}
                  >
                    <div onClick={(e) => { e.stopPropagation() }} className="w-56 bg-white border border-[var(--color-stroke-neutral)] divide-y divide-[var(--color-stroke-neutral)] rounded-lg shadow-[4px_4px_8px_0_var(--color-notif-shadow-soft),0px_0px_4px_0_var(--color-notif-shadow-strong)] z-50">
                      {(can("edit category", "support") || can("edit category")) && (
                        <Button
                          variant="profile"
                          ...
                          onClick={() => {
                            setMenuOpen(null);
                            setEditInitial({ id: cat.id, name: cat.title, icon: cat.iconId, description: cat.description, roles: cat.verticalId ? [cat.verticalId] : [] });
                            setEditOpen(true);
                          }}
                        >
                          <PencilLine className="w-5 h-5 text-[var(--color-neutral-light)]" /> Edit category
                        </Button>
                      )}
                      {(can("suspend categories", "support") || can("suspend categories")) && (
                        <Button
                          variant="profile"
                          ...
                          onClick={async () => {
                            setMenuOpen(null);
                            let count = typeof cat.faqCount === 'number' ? cat.faqCount : null;
                            if (count === null) {
                              try {
                                const res = await faqService.getFaqsByCategory(cat.id);
                                const faqs = res?.data?.faqs || res?.data?.data?.faqs || [];
                                count = Array.isArray(faqs) ? faqs.length : 0;
                              } catch (_) { count = 0; }
                            }
            setSuspendMeta({ title: cat.title, faqCount: count });
            setSuspendCategoryId(cat.id);
                            setSuspendOpen(true);
                          }}
                        >
                          <Icon name="user_wrong" className="w-5 h-5 text-[var(--color-neutral-light)]" /> Suspend category
                        </Button>
                      )}
                          ...
                          onClick={async () => {
                            ...
                            if (count === null) {
                              try {
                                const res = await faqService.getFaqsByCategory(cat.id);
                                const faqs = res?.data?.faqs || res?.data?.data?.faqs || [];
                                count = Array.isArray(faqs) ? faqs.length : null;
                              } catch (_) { count = null; }
                            }
                            setDeleteMeta({ title: cat.title, faqCount: count });
                            setDeleteCategoryId(cat.id);
                            setDeleteOpen(true);
                          }}
                        >
                          <Trash2 className="w-5 h-5 text-[var(--notif-error)]" /> Delete category
                        </Button>
                      )}
                    </div>
                  </DropdownPortal>
                ...
                {iconIdToKey[cat.iconId] ? (
                  <img src={`${iconBaseUrl}/${encodeURIComponent(iconIdToKey[cat.iconId])}`} alt="icon" className="w-8 h-8" />
                ) : (
                  <Icon name="notes_info" className="w-8 h-8 text-[var(--color-brand-icon)] group-hover:text-[var(--info-panel-view-bg)]" />
                )}
                <div className="text-[var(--color-neutral-primary)] font-semibold text-base break-words whitespace-normal text-center w-full">{cat.title}</div>
                <p className="text-sm text-[var(--color-stroke-brand)] line-clamp-2">{cat.description}</p>
                <div className="text-xs text-[var(--color-stroke-brand)]">(Visible to {verticalIdToName[cat.verticalId] || '—'})</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <SuspendCategoryModal
        ...
        onConfirm={async () => {
          try {
            if (!suspendCategoryId) { setSuspendOpen(false); return; }
            const res = await faqService.suspendCategories([suspendCategoryId]);
            if (res?.success && res?.code === 200) {
              const updated = categories.filter(c => c.id !== suspendCategoryId);
              setCategories(updated);
              setAllCategories(allCategories.filter(c => c.id !== suspendCategoryId));
              showSuccess("Success!", `Category suspended.`);
            } else {
              const err = res?.message || res?.error || "Failed to suspend category.";
              showError(err);
            }
          } catch (e) {
            const msg = e?.response?.data?.message || e?.message || "Failed to suspend category.";
            showError(msg);
          } finally {
      ...
      <DeleteCategoryModal
        ...
        onConfirm={async () => {
          try {
            if (!deleteCategoryId) { setDeleteOpen(false); return; }
            const res = await faqService.deleteCategories([deleteCategoryId]);
            if (res?.success && res?.code === 200) {
              const updated = categories.filter(c => c.id !== deleteCategoryId);
              setCategories(updated);
              setAllCategories(allCategories.filter(c => c.id !== deleteCategoryId));
              showSuccess("Deleted!", "Category deleted successfully.");
            } else {
              const err = res?.message || res?.error || "Failed to delete category.";
              showError(err);
            }
          } catch (e) {
