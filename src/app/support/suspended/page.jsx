...
export default function SuspendedSupportCategoriesPage() {
    ...
    const load = async () => {
      try {
        ...
        if (opts.length) {
          setVerticalOptions([
            { value: ALL_VERTICALS_OPTION.id, label: ALL_VERTICALS_OPTION.label },
            ...opts.map(opt => ({ value: opt.id, label: opt.label })),
          ]);
        } else {
  ...
  const searchData = useMemo(
    () => categories.map((c) => ({ id: c.id, name: c.title, code: verticalIdToName[c.verticalId] || '' })),
  ...
  const filtered = useMemo(() => {
    return categories.filter((c) => {
      ...
      const catVerticalId = c.verticalId !== undefined && c.verticalId !== null ? String(c.verticalId) : "";
  ...
  const exportOptions = [
    {
      ...
      items: [
        ...
        { id: "filtered", label: "As per the filtered list", type: "radio" },
      ],
    },
  ];
  ...
  const handleReactivate = async () => {
    try {
      const ids = selectedCategory ? [selectedCategory.id] : filtered.map(c => c.id);
      if (ids.length === 0) { setIsModalOpen(false); return; }
      const res = await faqService.reactivateCategories(ids);
      if (res?.success && res?.code === 200) {
        showSuccess('Success!', `${ids.length} ${ids.length === 1 ? 'category has' : 'categories have'} been reactivated.`);
        setCategories(prev => prev.filter(c => !ids.includes(c.id)));
      } else {
        const err = res?.message || res?.error || 'Failed to reactivate categories.';
        showError(err);
      }
    } catch (e) {
      const msg = e?.response?.data?.message || e?.message || 'Failed to reactivate categories.';
      showError(msg);
    } finally {
  ...
  const handleExportConfirm = async ({ scope, checked }) => {
    try {
      ...
      if (scope === 'filtered') {
        ...
        if (vertical && vertical !== 'all') params.vertical_id = vertical;
      }
      ...
      if (response && typeof response === 'object' && response.blob) {
        ...
        const filename = response.filename || `suspended_categories_${new Date().toISOString().split('T')[0]}.csv`;
      ...
      const errorMessage = error?.message || 'Failed to export suspended categories. Please try again.';
      showError(errorMessage);
    }
  };

  const canReactivate = (can("activate categories", "support") || can("activate categories"));
  const canEdit = (can("edit category", "support") || can("edit category"));
  const canDelete = (can("delete categories", "support") || can("delete categories"));
  const canExport = (can("export suspended_categories", "support") || can("export suspended_categories"));
    ...
    <div className="min-h-[calc(100vh-150px)]">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <Button variant="cancel" onClick={() => router.push("/support")} className="p-2 rounded-lg">
            <svg className="w-4 h-4 text-[var(--color-stroke-brand)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
          </Button>
          <h1 className="text-lg font-semibold text-[var(--color-neutral-primary)]">Suspended categories</h1>
        </div>
        {(loading || filtered.length > 0) && (
          <div className="flex items-center gap-4">
            {canReactivate && filtered.length > 0 && (
              <Button variant="secondary" className="btn-size-md-sm !px-3 font-medium" onClick={() => handleOpenReactivate()}>ACTIVATE ALL</Button>
            )}
            {canExport && filtered.length > 0 && (
              <Button variant="cancel" size="sm" onClick={() => setExportOpen(true)} className="btn-size-md-sm">EXPORT LIST</Button>
            )}
          </div>
        )}
      </div>
        ...
        <>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
              {filtered.map((cat) => (
                <div key={cat.id} className="border border-[var(--color-stroke-neutral)] group hover:border-[var(--color-admin-profile-border)] transition-colors rounded-lg bg-white p-6 flex flex-col gap-4 cursor-default">
                  <div className="relative flex flex-col items-center gap-3 text-center">
                    <div className="w-full flex justify-end">
                      {(canReactivate || canEdit || canDelete) && (
                        <button
                          ref={(el) => (buttonRefs.current[cat.id] = el)}
                          onClick={(e) => {
                            e.stopPropagation();
                            setMenuOpen(
                              menuOpen === cat.id ? null : cat.id
                            )
                          }
                          }
                            ...
                            <div className="w-56 bg-white border border-[var(--color-stroke-neutral)] divide-y divide-[var(--color-stroke-neutral)] rounded-lg shadow-[4px_4px_8px_0_var(--color-notif-shadow-soft),0px_0px_4px_0_var(--color-notif-shadow-strong)] z-50">
                              {canReactivate && (
                                <Button
                                  variant="profile"
                                  onClick={() => {
                                    handleOpenCategoryReactivate(cat);
                                    setMenuOpen(null);
                                  }}
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    setEditInitial({
                                      ...
                                      roles: cat.verticalId ? [cat.verticalId] : [],
                                    });
                                    setEditOpen(true);
                                  }}
                              ...
                              {canDelete && (
                                <Button
                                  ...
                                  onClick={() => {
                                    setMenuOpen(null);
                                    setDeleteMeta({ title: cat.title, faqCount: cat.faqCount || 0 });
                    ...
                    <Icon name={cat.icon || 'notes_info'} className="w-8 h-8 text-[var(--color-brand-icon)] group-hover:text-[var(--info-panel-view-bg)]" />
                    <div className="text-[var(--color-neutral-primary)] font-semibold text-base">{cat.title}</div>
          ...
          {editOpen && (
            <AddCategory
              ...
              onClose={async () => {
                ...
                setEditInitial(null);
                try {
                  const catsRes = await faqService.getCategories({ category_state: 'suspended' });
                  if (catsRes?.success && catsRes?.code === 200) {
                    const list = catsRes.data?.faq_categories || catsRes.data?.data?.faq_categories || [];
                    const normalized = list.map(c => ({
                      id: c.id,
